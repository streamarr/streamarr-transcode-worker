...
- **Fence stale sessions by identity**: callbacks that outlive their session compare the session object they captured with the current one and return when they differ (`sendProbeResult`, `reportStopped`, `endSession`). A disconnected or replaced session must never publish results, upload segments, or stop the next session's work. Commands whose target is not this worker id and boot id are rejected or ignored.
