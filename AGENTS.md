...
- `node buildpacks/ffmpeg/bin/vendor-notices.mjs` — regenerate `notices/sources.json`, the notice files, and `SOURCE.txt` from upstream's pins for the locked release; `--dry-run` only reports, and so does a run at the release `notices/manifest` binds. It never writes `notices/manifest`, and a changed inventory still needs a maintainer's review. A proposed component's distribution follows its recipe: one whose recipe pins one repository, runs `gen-implib` and removes the shared library (`rm` of a `.so`) is proposed as `embedded` (headers compiled in beside generated import shims; the system library is not bundled) and reported as `(import shim)`; any other as a static `runtime` library, reported as `(generates import shims; classify by hand)` when its recipe runs `gen-implib` for several pins or keeps the shared library. Its verdict compares upstream with the working tree, so it says "unchanged" only while `notices/sources.json` is still the inventory `notices/manifest` binds
