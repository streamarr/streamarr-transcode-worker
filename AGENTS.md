...
- `node buildpacks/ffmpeg/bin/vendor-notices.mjs` — regenerate `notices/sources.json`, the notice files, and `SOURCE.txt` from upstream's pins for the locked release; `--dry-run` only reports. It never writes `notices/manifest`, and a changed inventory still needs a maintainer's review. Its verdict compares upstream with the working tree, so it says "unchanged" only while `notices/sources.json` is still the inventory `notices/manifest` binds
