...
- Node tests cover the tooling around the build: `node --test buildpacks/ffmpeg/test/*.test.mjs` (notice generator), `node --test .github/actions/pack-build/*.test.mjs` (image build and publication boundaries), `node --test .github/actions/sonar/test/*.test.mjs` (analysis failure handling), `node --test src/test/fmp4-recorder/*.test.mjs` (fMP4 fixture recorder)
- `node src/test/fmp4-recorder/check-expectations.mjs` — check `src/test/resources/fmp4/expected.json` against the committed recordings offline (no Docker); CI runs it. `src/test/fmp4-recorder/record-fixtures.sh` re-records the recordings and `expected.json` with Docker and the pinned worker image. See the [fixture README](src/test/resources/fmp4/README.md)
- `node buildpacks/ffmpeg/bin/vendor-notices.mjs` — regenerate `notices/sources.json`, the notice files, and `SOURCE.txt` from upstream's pins for the locked release; `--dry-run` only reports, and so does a run at the release `notices/manifest` binds. It never writes `notices/manifest`, and a changed inventory still needs a maintainer's review. A proposed component's distribution follows its recipe: one that runs `gen-implib` is proposed as `embedded` (headers compiled in beside generated import shims; the system library is not bundled) and reported as `(import shim)`, any other as a static `runtime` library. Its verdict compares upstream with the working tree, so it says "unchanged" only while `notices/sources.json` is still the inventory `notices/manifest` binds
...
- **Fence stale sessions by identity**: callbacks that outlive their session compare the session object they captured with the current one and return when they differ (`sendProbeResult`, `reportClaimed`, `endSession`). A disconnected or replaced session must never publish results, upload segments, or stop the next session's work. Commands whose target is not this worker id and boot id are rejected or ignored.
- **Don't join work that needs the monitor while holding it**: each job attempt's producer uploads on its own virtual thread and takes the worker monitor only to check that its attempt is still active, so a stop claims the attempt under the monitor, where the producer records the stop before any upload can find the attempt inactive, then waits for FFmpeg to exit on its own virtual thread, which `close()` joins; the control stream never waits on FFmpeg. Probe completion needs the worker monitor, so `close()` joins probe work only after `closeConnection()` releases it.
