...
def oracle(run, flags, audio=True, reference=None, restrict=False, expect=True):
    """flags: "today" (source timestamps, no -start_at_zero) or "new" (-start_at_zero).
...
FIXTURES = [
    dict(name="01-encode-cfr", source="cfr.mp4", mode="encode", encoder="libx264", seek=0, start=0,
         ...
         oracles=[oracle("01-encode-cfr.today", "today"),
                  oracle("01-encode-cfr.video-only", "new", audio=False),
    ...
    dict(name="02-copy-irregular-keyframes", source="irregular.mp4", mode="copy", encoder=None, seek=0, start=0,
         ...
         oracles=[oracle("02-copy-irregular-keyframes.today", "today"),
                  oracle("02-copy-irregular-keyframes.video-only", "new", audio=False)]),
    dict(name="03-encode-vfr", source="vfr.mp4", mode="encode", encoder="libx264", seek=0, start=0,
         ...
         oracles=[oracle("03-encode-vfr.today", "today", expect=False),
                  oracle("03-encode-vfr.video-only", "new", audio=False, expect=False)]),
    dict(name="04-copy-vfr-bframes", source="vfr.mp4", mode="copy", encoder=None, seek=0, start=0,
         ...
         oracles=[oracle("04-copy-vfr-bframes.today", "today", expect=False),
                  oracle("04-copy-vfr-bframes.video-only", "new", audio=False, expect=False)]),
    dict(name="05-encode-late-start", source="late.ts", mode="encode", encoder="libx264", seek=0, start=0,
         ...
         oracles=[oracle("05-encode-late-start.today", "today", expect=False),
    ...
    dict(name="09-svtav1-vfr", source="vfr.mp4", mode="encode", encoder="libsvtav1", seek=0, start=0,
         ...
         oracles=[oracle("09-svtav1-vfr.video-only", "new", audio=False, expect=False),
                  oracle("09-svtav1-vfr.today", "today", expect=False),
                  oracle("09-svtav1-vfr.new-keyframes-with-audio", "new", expect=True)]),
...
def evaluate_oracle(work, fixture, spec, pipe, grouped, source):
    ...
    offset = source["start"] if spec["flags"] == "today" else Fraction(0)
    ...
    return {
        "hlsRun": spec["run"],
        "flags": "today's FfmpegCommandBuilder common flags" if spec["flags"] == "today"
        else "ADR 0037 common flags (-start_at_zero)",
