...
every invocation. Neither moves a keyframe or a cut. Nothing else differs from the recipe.
...
| 1b | `01-encode-cfr-seek30.fmp4` | 5 | Encoded seek: starts at 30.030 s (segment 5), 863 frames, nothing padded from zero, no preroll. Segments 5–10 start on exactly the ticks of the start-0 recording's segments 5–10. Ends with one audio-only fragment. |
...
| 3 | `03-encode-vfr.fmp4` | 0 | VFR source (1068 frames, avg 16.2 fps on the 23.976 grid, first frame at 41.7 ms) encoded under `-r`: 1582 constant-rate frames, the same count as the HLS recipe's output. A keyframe in every interval, 11 segments. |
...
| 1b | agrees with the start-0 recording's HLS output | differs on 7–10 by one frame | see below |
...
2. **Seek recordings** (1b, 9b, 7b). hlsenc measures from the seek's first frame (30.030 s, 30.072 s,
   ...
   so the grid opens segment 7 there (as the start-0 recording does). hlsenc cuts at the forced keyframe
   at 42.042 s. For 7b this is exactly the misalignment ADR 0037 describes for a replacement attempt.
...
- **"Grouping reproduced the HLS muxer's cut points … 11 into 11 for the variable-frame-rate
  ...
  - the pipe recipe's 143-frame GOP adds a keyframe at N·P + 6 ms;
...
- **"At 23.976 fps for encode, stream copy and a seek the two rules agree."** Encode and copy agree
  ...
  agrees tick for tick with the start-0 recording. With `-g 143`, libx264's GOP keyframe at 42.0003 s falls
...
- **An encode can end in a 1-frame segment past the source's content** (5a). The 30 s source's last
  ...
  frame. Whether the server advertises segment 5 depends on how it rounds the probed duration, and
  what happens to an upload of a number it does not advertise is not settled.
